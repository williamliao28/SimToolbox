...
void CPSolver::CPReadA(std::string filename) {
    // set up comm
    commRcp = getMPIWORLDTCOMM();
    // read file
...
// read A and b generated by the constructor from file
void CPSolver::CPReadb(int localSize, std::string filename) {
    // set up comm
    commRcp = getMPIWORLDTCOMM();
    // set map
    Teuchos::RCP<const TMAP> rowMapRcp = getTMAPFromLocalSize(localSize, commRcp);
    ...
    bRcp = matDumper.readVectorFile(filename, commRcp, mapRcp, false, false);
